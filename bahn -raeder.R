...
length(grep("BikeNumber", raw$V1,))  
## number of trips 403!

bikes <-  data.frame(cbind(raw[grep("StartTime", raw$V1,),]$V1,raw[grep("BikeNumber", raw$V1,),]$V1))
bikes$start = str_trim(str_split_fixed(bikes$X1, "=", 2)[,2])
bikes$nr = str_trim(str_split_fixed(bikes$X2, "=", 2)[,2])

bikes$startT <- as.POSIXct(bikes$start, format="%Y-%m-%d %H:%M:%S", tz = "CET")

agg <- ddply(bikes, .(nr), summarise, freq=length(nr))
max(agg$freq)  ## 6 trips!

ddply(agg, .(freq), summarise, num = length(freq), perc = round(length(freq)/ nrow(agg)*100,2) )

agg<- agg[order(agg$freq),]

ggplot(agg, aes(x=freq, color=1, fill=1)) + geom_density(alpha=.3)

### can i estimate the total number of bikes in ffm?
# total number of bikes
length(unique(bikes$nr))  # 277
nrow(bikes)     # 403

## a priori prob. that I will get a bike a second time (after one year sampling); 32 %

## eg. % of sampled bikes 32% ?!

## number of total bikes  ~ 870?
277/0.32


########### is using the same bike linked?!
### time difference

twice <- bikes[bikes$nr %in% agg[agg$freq==2,]$nr,]

twice <- twice[order(twice$startT),]
