...
class PyABike:
    ...
    def listCompletedTrips(self, firstEntry=0, entryCount=20, startTime='', endTime='', user='', passwd=''):
        ...
        if self.buildCustomerData(user, passwd):
            try:
                self.requestResponse = getattr(self.client.service, 'CABSERVER.listCompletedTrips')(
                    CommonParams=self.commonParams, CustomerData=self.customerData, TripLimits=self.tripLimits)
...
def buildNewCustomerData(self, productID, name, surname, sex, birthday, street, number, town, zipCode, countryCode,
                         email, phone):
...
def listFreeBikes(self, maxRes=100, radius=5000, longitude=0.0, latitude=0.0):
    if self.buildGeoPos(longitude, latitude):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.listFreeBikes')(
                CommonParams=self.commonParams, SearchPosition=self.geoPos, maxResults=maxRes, searchRadius=radius)
            return self.requestResponse
        except Exception as e:
            print "An Error occurred during the request: "
            print e
...
def getCustomerInfo(self, user='', passwd=''):
    ...
    if self.buildCustomerData(user, passwd):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.getCustomerInfo')(
                CommonParams=self.commonParams, CustomerData=self.customerData)
            return self.requestResponse
        except Exception as e:
            print e
    else:
...
def listReturnLocations(self, bike, maxRes=100, radius=5000, longitude=0.0, latitude=0.0):
    ...
    if self.buildGeoPos(longitude, latitude):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.listReturnLocations')(
                CommonParams=self.commonParams, BikeNumber=bike, SearchPosition=self.geoPos, maxResults=maxRes,
                searchRadius=radius)
            return self.requestResponse
        except Exception as e:
            print "An Error occurred during the request: "
...
def rentBike(self, bike, user='', passwd=''):
    ...
    if self.buildCustomerData(user, passwd):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.rentBike')(CommonParams=self.commonParams,
...
def returnBike(self, bike, retCode, locID, user='', passwd=''):
    ...
    try:
        self.requestResponse = getattr(self.client.service, 'CABSERVER.returnBike')(CommonParams=self.commonParams,
                                                                                    BikeNumber=bike, ReturnCode=retCode,
                                                                                    LocationUID=locID,
                                                                                    CustomerDataOptional=self.customerData)
        return self.requestResponse
    except Exception as e:
        print "An Error occurred during the request: "
        print e
...
def requestNewPassword(self, phone):
    ...
    try:
        self.requestResponse = getattr(self.client.service, 'CABSERVER.requestNewPassword')(
            CommonParams=self.commonParams, PhoneNumber=phone)
        return self.requestResponse
    except Exception as e:
        print "An Error occurred during the request: "
        print e
...
def checkTripStart(self, bike, user='', passwd=''):
    ...
    if self.buildCustomerData(user, passwd):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.checkTripStart')(
                CommonParams=self.commonParams, CustomerData=self.customerData, BikeNumber=bike)
            return self.requestResponse
        except Exception as e:
            print "An Error occurred during the request: "
            print e
...
def changePersCode(self, code, user='', passwd=''):
    ...
    if self.buildCustomerData(user, passwd):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.changePersCode')(
                CommonParams=self.commonParams, CustomerData=self.customerData, persCode=code)
            return self.requestResponse
        except Exception as e:
            print "An Error occurred during the request: "
            print e
...
def getBikeInfo(self, bike):
    ...
    #BikeNumber: https://xml.dbcarsharing-buchung.de/hal2_cabserver/:Type_BikeNumber
    try:
        self.requestResponse = getattr(self.client.service, 'CABSERVER.getBikeInfo')(CommonParams=self.commonParams,
                                                                                     BikeNumber=bike)
        return self.requestResponse
    except Exception as e:
        print "An Error occurred during the request: "
...
def redeemBonusCode(self, bonusCode, user='', passwd=''):
    ...
    if self.buildCustomerData(user, passwd):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.changePersCode')(
                CommonParams=self.commonParams, CustomerData=self.customerData, BonusCode=bonusCode)
            return self.requestResponse
        except Exception as e:
            print "An Error occurred during the request: "
            print e
...
def addCustomer(self):
    ...
    if hasattr(self, 'newCustomerData'):
        try:
            self.requestResponse = getattr(self.client.service, 'CABSERVER.addCustomer')(CommonParams=self.commonParams,
                                                                                         NewCustomerData=self.newCustomerData)
            return self.requestResponse
        except Exception as e:
            print "An Error occurred during the request: "
            print e
...
def reportDamage(self, damageText='', bike=0, locID=0, user='', passwd=''):
    ...
    if self.buildDamageData(damageText, bike, locID):
        if self.self.buildCustomerData(user, passwd):
            try:
                self.requestResponse = getattr(self.client.service, 'CABSERVER.reportDamage')(
                    CommonParams=self.commonParams, CustomerData=self.customerData, DamageData=self.damageData)
                return self.requestResponse
            except Exception as e:
                print "An Error occurred during the request: "
                print e
